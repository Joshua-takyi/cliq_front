...
export async function PATCH(req: Request) {
  try {
    ...
    const convertedId = mongoose.Types.ObjectId.isValid(id)
    ...
    const statusMessages: Record<DeliveryStatus, string> = {
      processing:
        "Order status updated to processing and email sent successfully",
      shipped: "Order status updated to shipped and email sent successfully",
      delivered:
        "Order status updated to delivered and email sent successfully",
      cancelled:
        "Order status updated to cancelled and email sent successfully",
    ...
    if (deliveryStatus !== "pending" && existingOrder.shippingInfo?.email) {
      ...
      const templateFn =
        templates[deliveryStatus as Exclude<DeliveryStatus, "pending">];
      if (templateFn) {
